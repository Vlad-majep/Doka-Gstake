...
_POST_ `api/v1/auth/admin/signin` - авторизация администратора
...
_GET_ `api/v1/user/me` - вывод текущего пользователя
...
_GET_ `api/v1/user/` - вывод пользователей
...
_GET_ `api/v1/user/:userId` - вывод пользователя
...
_POST_ `api/v1/deposit/:currency` - депозит сети (eth + bsc) / btc
...
_POST_ `api/v1/two_factor_authentication` - включить двух факторную аутентификацию
...
_DELETE_ `api/v1/two_factor_authentication` - выключить двух факторную аутентификацию
...
_GET_ `api/v1/two_factor_authentication` - получить qr code
...
_POST_ `transaction` - добавить транзакции в базу данных
...
_GET_ `transaction` - вывести транзакции
